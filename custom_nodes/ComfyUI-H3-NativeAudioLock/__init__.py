...
class MiniMaxH3NativeAudioLock:
    """Replace H3's target audio latent with user audio and denoise video only."""
    ...
    DESCRIPTION = (
        "Encodes exact user audio into H3's target AV latent, fixes the audio stream at its clean "
        "timestep, and masks sampling so H3 denoises only video while jointly attending to the song."
    )
